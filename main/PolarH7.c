...
static int esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param);
static int gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param);
...
bool is_polar_h7_device(uint8_t *adv_name, uint8_t adv_name_len, uint8_t *adv_data, uint8_t adv_data_len)
{
    // Check if the device name matches your Polar H7 device name
    if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0)
    {
        // Check if the service UUID is present in the advertisement data
        esp_bt_uuid_t service_uuid = {
            .len = ESP_UUID_LEN_16,
            .uuid = {.uuid16 = REMOTE_SERVICE_UUID},
        };
        return esp_ble_resolve_adv_data(adv_data, adv_data_len, &service_uuid);
    }
    return false;
}
